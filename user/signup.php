...
if(isset($_POST['userreg'])){
    ...
    if(!isset($_POST['accept'])) {
        header("Location: /public_html/pages/registration.php?error=terms");
        exit();
    }

    // Validate email
    if (!$mail) {
        header("Location: /public_html/pages/registration.php?error=email");
        exit();
    }

    // Password requirements
    if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $password)) {
        header("Location: /public_html/pages/registration.php?error=password");
        exit();
    }

    // Check if passwords match
    if ($password !== $confirmPassword) {
        header("Location: /public_html/pages/registration.php?error=password_match");
        exit();
    }
    ...
    try {
        ...
        if($stmt->fetchColumn() > 0) {
            header("Location: /public_html/pages/registration.php?error=exists");
            exit();
        }
        ...
        header("Location: /public_html/pages/registration.php?error=database");
        exit();
    }
}
?>